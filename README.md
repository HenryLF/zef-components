...
```typescript
interface BaseComponent<T extends object, K = unknown>
  extends HTMLElement {
  rawHTML: string;
...
- Function that returns the component's current value, will be called on every state change.
- Triggers an "input" event when the value changes

### onMount
Callback executed when the component is connected to the DOM. Can be asynchronous.

**Type:**
```typescript
(this: BaseComponent<T>) => void | Promise<void>
```
...
**Type:**
```typescript
type EventListernerRecord<T extends object> = Record<
  string,
  EventHandle<T>[]
>
```
...
- `{{property.path?:'false text'}}` - Ternary with only false case (true case empty)
## Event Handling
...
6. **Async Operations**: Use the async `onMount` for data fetching and other asynchronous initialization